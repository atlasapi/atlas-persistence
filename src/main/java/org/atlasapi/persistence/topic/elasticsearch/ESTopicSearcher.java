...
public class ESTopicSearcher implements TopicSearcher {
    ...
    @Override
    public List<Topic> popularTopics(Interval interval, TopicQueryResolver resolver) {
        Facets facets = buildQuery(interval).actionGet(timeout, TimeUnit.MILLISECONDS).getFacets();
        TermsFacet terms = facets.facet(TermsFacet.class, ESItem.TOPICS);
        List<Topic> result = new LinkedList<Topic>();
        for (TermsFacet.Entry term : terms.entries()) {
    ...
    private ListenableActionFuture<SearchResponse> buildQuery(Interval interval) {
        ListenableActionFuture<SearchResponse> result = index.client().prepareSearch(ESSchema.INDEX_NAME).setQuery(
                ...
                addFacet(FacetBuilders.termsFacet(ESItem.TOPICS).field(ESItem.TOPICS + "." + ESTopic.ID)).
                execute();
        return result;
    }
