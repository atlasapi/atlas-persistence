    ...
    private Iterable<ProductLocation> fromDBObject(List<DBObject> dbObjectList) {
        return Lists.transform(dbObjectList, new Function<DBObject, ProductLocation>() {
            ...
            public ProductLocation apply(DBObject input) {
                return ProductLocation.builder(TranslatorUtils.toString(input, URI_KEY)).withAvailability(TranslatorUtils.toString(input, AVAILABILITY_KEY))
                        .withPrice(fromDBObject(TranslatorUtils.toDBObject(input, PRICE_KEY))).withShippingPrice(fromDBObject(TranslatorUtils.toDBObject(input, SHIPPING_PRICE_KEY))).build();
            }
        });
    }
