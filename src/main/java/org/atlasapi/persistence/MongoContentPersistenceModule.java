...
public class MongoContentPersistenceModule implements ContentPersistenceModule {
    ...
    ContentWriter contentWriter() {
        ...
        if(Boolean.valueOf(generateIds)) {
            contentWriter = new IdSettingContentWriter(lookupStore(),
                    new MongoSequentialIdGenerator(db, "content"), contentWriter);
        }
    ...
    public @Primary
    @Bean
    MongoContentLister contentLister() {
    ...
    TopicStore topicStore() {
        return new TopicCreatingTopicResolver(new MongoTopicStore(db),
    ...
    SegmentWriter segmentWriter() {
        return new IdSettingSegmentWriter(new MongoSegmentWriter(db,
                new SubstitutionTableNumberCodec()), segmentResolver(),
    ...
    ChannelStore channelStore() {
        if(processingConfig == null || !processingConfig.toBoolean()) {
            return new CachingChannelStore(new MongoChannelStore(db, channelGroupStore(),
                    channelGroupStore()));
        }
